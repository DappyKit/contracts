...
describe('SocialConnections Upgradeability', () => {
  it('should preserve state and add new functionality after upgrade', async () => {
    const [owner] = await ethers.getSigners()

    // Deploy the initial contract
    const SocialConnectionsV1 = await ethers.getContractFactory('SocialConnections')
    const socialConnectionsV1 = await upgrades.deployProxy(SocialConnectionsV1, [owner.address], {
    ...
    await expect(upgrades.upgradeProxy(socialConnectionsV2, SocialConnectionsV1)).to.eventually.be.rejectedWith(
      'New storage layout is incompatible',
    )
  })

  it('should transfer ownership and allow only new owner to upgrade the contract', async () => {
    const [owner1, owner2] = await ethers.getSigners()

    // Deploy the initial contract
    const SocialConnectionsV1 = await ethers.getContractFactory('SocialConnections')
    const socialConnectionsV1 = await upgrades.deployProxy(SocialConnectionsV1, [owner1.address], {
      initializer: 'initialize',
    })
    await socialConnectionsV1.waitForDeployment()
